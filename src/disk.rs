...
impl DiskManager {
    pub fn new(heap_file: File) -> io::Result<Self> {
        let heap_file_size = heap_file.metadata()?.len(); // file size
    ...
    pub fn read_page_data(&mut self, page_id: PageId, data: &mut [u8]) -> io::Result<()> {
        ...
        self.heap_file.seek(SeekFrom::Start(offset))?; // place cursor at offset
        self.heap_file.read_exact(data) // read data from file as buf can be contained
    }
...
mod tests {
    ...
    fn test() {
        ...
        disk.write_page_data(hello_page_id, &mut hello).unwrap();
        ...
        disk.write_page_data(world_page_id, &mut world).unwrap();
        drop(disk);
